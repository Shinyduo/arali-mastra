...
export const assignKeyRole = createTool({
  ...
  inputSchema: z.object({
    ...
    roleKey: z
      .string()
      .describe("Role key (e.g. 'csm', 'ae', 'tam')"),
    newOwnerEmail: z
      .string()
      .email()
  ...
  suspendSchema: z.object({
    action: z.literal("assign-key-role"),
    summary: z.string(),
  }),
  ...
  execute: async (input, context) => {
    ...
    if (context.agent?.resumeData) {
      ...
      if (!resume.approved) {
        return { success: false, message: "Role assignment cancelled." };
      }
      ...
      // End current assignment (set endAt)
      await db
